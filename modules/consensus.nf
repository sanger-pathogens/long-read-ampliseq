process CURATE_CONSENSUS {
    ...
    script:
    """
    gvcf_to_fasta.py -r ${reference} -v ${gvcf_final} --fasta_id ${meta.ID} -b ${target_regions_bed} -n --multi_locus --multifasta -o ${meta.ID} --min_ref_gt_qual ${params.min_ref_gt_qual} --min_alt_gt_qual ${params.min_alt_gt_qual}
    # below outputs per-sample, per-locus fasta files THAT HAVE REF BASE WHEN UNDEFINED I.E. NOT AN N
