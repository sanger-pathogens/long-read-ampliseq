process CLAIR3_CALL {
    ...
    publishDir "${params.outdir}/variants/", mode: 'copy', overwrite: true, pattern: 'merge_output.vcf.gz', saveAs: { filename -> "${meta.ID}_clair3.gvcf.gz" }
    ...
    input:
    tuple val(meta), path(sorted_bam), path(bam_index), path(target_regions_bed), path(reference), path(reference_index)
